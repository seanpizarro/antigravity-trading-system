...
class OpportunityScanner:
    ...
    def scan_opportunities(self) -> List[Dict]:
        ...
        if not is_market_open():
            ...
            # Log the mock opportunities
            for opp in opportunities:
                self.logger.info(f"🎯 MOCK OPPORTUNITY: {opp['symbol']} {opp['strategy']} - ${opp['premium']:.2f}")
        ...
        for opportunity in opportunities:
            try:
                ...
                if opportunity.get('data_source') == 'mock_after_hours':
                    ...
                    ai_confidence = 0.65 + (hash(opportunity['symbol']) % 100) * 0.003
                ...
                self.logger.error(f"❌ Error analyzing opportunity {opportunity.get('symbol')}: {e}")
