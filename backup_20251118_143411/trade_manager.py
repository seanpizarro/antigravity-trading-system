...
class ActiveTradeManager:
    ...
    def manage_all_positions(self, positions: Dict) -> List[ManagementAction]:
        ...
        for position_id, position in positions.items():
            try:
                ...
                position['entry_date'] = position.get('entry_date') or position.get('entry_time', 'UNKNOWN')
                if 'strategy_type' not in position: position['strategy_type'] = position.get('strategy', 'UNKNOWN')
                if 'dte' not in position: position['dte'] = 0
                if 'current_pnl' not in position: position['current_pnl'] = 0

                # Check if position is too new to manage
                if self._is_position_too_new(type('Position', (), position)):
                    continue

                # Get market conditions
                market_conditions = self._get_current_market_conditions(type('Position', (), position))
                
                # Calculate real-time metrics using JAX
                # Ensure position dict has necessary fields for JAX
                jax_position = position.copy()
                if 'legs' not in jax_position:
                    # Create dummy legs if missing (fallback)
                    jax_position['legs'] = [{'strike': 0, 'option_type': 'call', 'quantity': 1}]
                
                metrics = self.jax_engine.calculate_position_metrics(jax_position)
                
                # Check for emergency conditions first
                emergency_action = self._check_emergency_conditions(type('Position', (), position), metrics)
                if emergency_action:
                    actions.append(emergency_action)
                    continue
                
                # Get DeepSeek management decision
                decision = self.deepseek_ai.analyze_position_management(
                    type('Position', (), position), metrics, market_conditions
                )
                
                # Validate and execute decision
                if self._should_execute_management(decision, type('Position', (), position)):
                    action = self._create_management_action(decision, type('Position', (), position), metrics)
                    actions.append(action)
                    self.daily_action_count += 1
                    
                    if self.daily_action_count >= self.management_rules['max_daily_management_actions']:
                        self.logger.warning("⚠️ Daily management action limit reached")
                        break
    ...
    def _is_position_too_new(self, position: object) -> bool:
        """Check if position is too new to manage (avoid overtrading)"""
        # 🎯 FIX: Handle missing entry_time gracefully
        # position is an object created via type('Position', (), dict)
        # so we use getattr
    ...
    def _get_current_market_conditions(self, position: Dict) -> Dict:
        ...
        return {
            ...
            'sector_performance': self._get_sector_performance(getattr(position, 'sector', 'SPY')),
            'volatility_regime': self._get_volatility_regime(),
            'economic_events': self._get_upcoming_events(getattr(position, 'ticker', 'SPY'))
        }
    ...
    def _should_execute_management(self, decision: ManagementDecision, 
        ...
        recent_actions = self._get_recent_actions_for_position(getattr(position, 'position_id', 'unknown'))
    ...
    def _validate_close_decision(self, decision: ManagementDecision, 
        ...
        profit_target = self.management_rules['profit_targets'].get(
            getattr(position, 'strategy_type', 'credit_spread'), 0.5
        )
        ...
        if getattr(position, 'dte', 30) <= 7:  # 7 days to expiration
            return True
    ...
    def _validate_roll_decision(self, decision: ManagementDecision, 
        ...
        # Check DTE threshold
        if getattr(position, 'dte', 30) > self.management_rules['time_decay_threshold']:
            return False
    ...
    def _is_thesis_broken(self, position: Dict) -> bool:
        ...
        current_price = getattr(position, 'underlying_price', 0)
        entry_price = getattr(position, 'entry_underlying_price', current_price)
        if entry_price == 0: return False

        # For credit spreads, thesis broken if price approaches short strike
        if getattr(position, 'strategy_type', '') == 'CREDIT_SPREAD':
            legs = getattr(position, 'legs', [])
            if not legs: return False
            short_strike = min([leg['strike'] for leg in legs])
    ...
    def _improves_position(self, position: Dict, adjust_params: Dict) -> bool:
        """Check if adjustment improves position"""
        # Convert object to dict for JAX engine if needed, or ensure JAX engine handles objects
        # Assuming JAX engine expects dict
