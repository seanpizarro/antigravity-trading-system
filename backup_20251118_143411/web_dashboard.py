...
class TradingDashboard:
    ...
    def initialize_session_state(self):
        ...
        if 'system_status' not in st.session_state:
            st.session_state.system_status = {'tasty': False, 'deepseek': False, 'jax': False}

    def check_system_health(self):
        """Check health of system components"""
        status = {'tasty': False, 'deepseek': False, 'jax': False}
        
        # Check TastyTrade
        try:
            if dual_tasty_api.get_account('paper'):
                status['tasty'] = True
        except:
            pass
        ...
        # Check JAX
        try:
            if st.session_state.jax_engine:
                status['jax'] = True
        except:
            pass
    ...
    def render_system_status(self):
        ...
        s = st.session_state.system_status
        
        with st.sidebar:
            st.markdown("### 🖥️ System Status")
            
            c1, c2 = st.columns([3, 1])
            with c1: st.write("TastyTrade API")
            with c2: st.write("🟢" if s['tasty'] else "🔴")
            
            c1, c2 = st.columns([3, 1])
            with c1: st.write("DeepSeek AI")
            with c2: st.write("🟢" if s['deepseek'] else "🔴")
            
            c1, c2 = st.columns([3, 1])
            with c1: st.write("JAX Engine")
            with c2: st.write("🟢" if s['jax'] else "🔴")
            
            if not all(s.values()):
                st.error("System components offline")
    ...
    def render_sidebar(self):
        ...
        with st.sidebar:
            ...
            if live_enabled:
                st.warning("⚠️ Execution: **ENABLED**")
            else:
                st.info("🛡️ Execution: **SAFETY LOCK**")
        ...
        with col_scan:
            ...
            if st.button("⭐ Prioritize with AI", use_container_width=True):
                if st.session_state.opportunities:
                    with st.spinner("DeepSeek AI ranking opportunities..."):
                        # Prepare data
                        opps = [o if isinstance(o, dict) else o.__dict__ for o in st.session_state.opportunities]
                        positions = dual_tasty_api.get_positions()
                        flat_positions = {}
                        for acct, pos_dict in positions.items():
                            flat_positions.update(pos_dict)
                        
                        risk_params = {'max_risk_per_trade': 500, 'max_open_positions': 5}
                        
                        # Call DeepSeek
                        prioritized = st.session_state.risk_monitor.deepseek_ai.prioritize_opportunities(
                            opps, flat_positions, risk_params
                        )
                        
                        # Update session state with prioritized list (converted back to dicts if needed)
                        st.session_state.opportunities = [p.__dict__ for p in prioritized]
                else:
                    st.warning("Scan market first.")
    ...
    def render_ai_insights(self):
        ...
        with col1:
            ...
            st.divider()
            st.subheader("🕵️ Position Management Analysis")
            if st.button("Analyze Open Positions", key="analyze_positions"):
                with st.spinner("DeepSeek AI evaluating position management..."):
                    # Get positions
                    positions = dual_tasty_api.get_positions()
                    flat_positions = []
                    for acct, pos_dict in positions.items():
                    ...
                    if not flat_positions:
                        ...
                        # Analyze each position (limit to top 5 for demo/speed)
                        analysis_results = []
                        for pos in flat_positions[:5]:
                            # Mock metrics for now (in real app, calculate these)
                            metrics = type('Metrics', (), {
                                'delta': 0.15, 'theta': -0.05, 'gamma': 0.01, 'vega': 0.02,
                                'pop': 0.75, 'days_since_entry': 5
                            })
                            # Mock market conditions
                            market_cond = {'vix': 15.5, 'market_trend': 'bullish'}
                            
                            # Normalize position data for DeepSeek
                            pos['ticker'] = pos.get('ticker') or pos.get('symbol', 'UNKNOWN')
                            pos['entry_date'] = pos.get('entry_date') or pos.get('entry_time', 'UNKNOWN')
                            if 'strategy_type' not in pos: pos['strategy_type'] = pos.get('strategy', 'UNKNOWN')
                            if 'dte' not in pos: pos['dte'] = 0
                            if 'current_pnl' not in pos: pos['current_pnl'] = 0

                            # Convert dict to object if needed by DeepSeek or handle in DeepSeek
                            # The DeepSeek method expects a Dict for position
                            decision = st.session_state.risk_monitor.deepseek_ai.analyze_position_management(
                                type('Position', (), pos), metrics, market_cond
                            )
                        ...
                        st.session_state['position_analysis'] = analysis_results
            
            if 'position_analysis' in st.session_state:
                st.dataframe(
                    pd.DataFrame(st.session_state['position_analysis']),
                    use_container_width=True,
                    hide_index=True
                )
    ...
    def render_performance(self):
        ...
        # Get trade history
        if hasattr(dual_tasty_api, 'get_history'):
            history = dual_tasty_api.get_history()
        else:
            history = []
            
        if not history:
            st.info("No closed trades yet. Performance data will appear here after your first closed trade.")
            return
        ...
        c1, c2, c3, c4 = st.columns(4)
        with c1: st.metric("Total P&L", f"${total_pnl:,.2f}", delta=f"{total_pnl:.2f}")
        with c2: st.metric("Win Rate", f"{win_rate:.1f}%")
        with c3: st.metric("Profit Factor", f"{profit_factor:.2f}")
        with c4: st.metric("Total Trades", len(df))
        ...
        with col_charts1:
            ...
            fig_pnl = px.line(df, x='exit_date', y='cumulative_pnl', title='Cumulative P&L', markers=True)
            fig_pnl.update_layout(xaxis_title="Date", yaxis_title="P&L ($)")
            st.plotly_chart(fig_pnl, use_container_width=True)
            
        with col_charts2:
            # Win/Loss Distribution
            fig_dist = px.histogram(df, x='pnl', nbins=20, title='P&L Distribution', color_discrete_sequence=['#1f77b4'])
            fig_dist.update_layout(xaxis_title="P&L ($)", yaxis_title="Count")
            st.plotly_chart(fig_dist, use_container_width=True)
            
        # Trade History Table
        st.subheader("📜 Trade History")
        st.dataframe(
            df[['symbol', 'strategy', 'status', 'entry_price', 'exit_price', 'pnl', 'exit_time']].sort_values('exit_time', ascending=False),
            column_config={
                "entry_price": st.column_config.NumberColumn("Entry", format="$%.2f"),
                "exit_price": st.column_config.NumberColumn("Exit", format="$%.2f"),
                "pnl": st.column_config.NumberColumn("P&L", format="$%.2f"),
                "exit_time": st.column_config.DatetimeColumn("Closed At", format="D MMM YYYY, HH:mm"),
            },
            use_container_width=True,
            hide_index=True
        )
    ...
    def run(self):
        ...
        self.render_system_status() # Add system status check
        self.render_header()
        ...
        # Main Tabs
        tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["🎯 Opportunities", "💼 Portfolio", "📊 Strategy Visualizer", "🛡️ Risk & Analytics", "🧠 AI Insights", "📈 Performance"])
