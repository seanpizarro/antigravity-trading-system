            ...
            for account_type in target_accounts:
                ...
                if risk_monitor and risk_monitor.approve_trade(opportunity, self.open_positions.get(account_type, {})):
                    ...
                    if execution_result.get('success'):
                        self.logger.info(f"✅ Executed trade on {account_type}: {opportunity.get('symbol', 'Unknown')}")
                        
                        # Update positions
                        position_id = execution_result.get('position_id', f"{opportunity.get('symbol')}_{datetime.now().timestamp()}")
