...
class AccountRiskMonitor:
    ...
    def calculate_dynamic_position_size(self, opportunity: Dict, account_balance: float) -> int:
        ...
        confidence = getattr(opportunity, 'ai_confidence', 0.5)
        iv = getattr(opportunity, 'implied_volatility', 20)
        premium = getattr(opportunity, 'premium', 0.50)  # Default to $0.50 if missing
    ...
    def _check_risk_limits(self, opportunity: Dict) -> bool:
        ...
        trade_risk = getattr(opportunity, 'max_risk', 0)
    ...
    def approve_trade(self, opportunity: Dict, current_positions: Dict) -> bool:
        ...
        try:
            ...
            if getattr(opportunity, 'data_source', '').startswith('mock'):
    ...
    def _calculate_portfolio_greeks(self, open_positions: Dict) -> Dict[str, float]:
        """Calculate portfolio-level Greeks using JAX engine"""
        try:
            if not open_positions:
                return {'delta': 0.0, 'gamma': 0.0, 'theta': 0.0, 'vega': 0.0}

            # Prepare data for JAX vectorized calculation
            ...
            for pos in open_positions.values():
                ...
                if not pos.get('strike') or not pos.get('expiration'):
                    continue
                    
                # Extract data
                underlying_prices.append(pos.get('underlying_price', 0))
            ...
            # Create JAX data structure
            positions_data = {
                'underlying_price': jnp.array(underlying_prices),
                'strikes': jnp.array(strikes),
                'expirations': jnp.array(expirations),
                'volatilities': jnp.array(volatilities),
                'rates': jnp.array(rates),
                'option_types': jnp.array(option_types),
                'quantities': jnp.array(quantities)
            }
            
            # Use JAX engine
            if not hasattr(self, 'jax_engine'):
                from jax_engine import JAXRealTimeAnalytics
                self.jax_engine = JAXRealTimeAnalytics()
                
            greeks = self.jax_engine.calculate_spread_greeks(positions_data)
            ...
            self.logger.error(f"JAX Greek calculation failed: {e}")
            # Fallback to simple summation if available
            return {
                'delta': sum(pos.get('delta', 0) for pos in open_positions.values()),
                'gamma': sum(pos.get('gamma', 0) for pos in open_positions.values()),
                'theta': sum(pos.get('theta', 0) for pos in open_positions.values()),
                'vega': sum(pos.get('vega', 0) for pos in open_positions.values())
            }
    ...
    def _calculate_sector_concentration(self, open_positions: Dict) -> Dict[str, float]:
        ...
        for position in open_positions.values():
            ...
            sector_values[sector] = sector_values.get(sector, 0) + getattr(position, 'current_value', 0)
    ...
    def _get_vix_level(self) -> float:
        ...
        try:
            ...
            vix = yf.Ticker("^VIX")
            # Get the most recent closing price or current price
            history = vix.history(period="1d")
            if not history.empty:
                return history['Close'].iloc[-1]
            return 18.5  # Fallback if no data
        except Exception as e:
            self.logger.warning(f"⚠️ Failed to fetch VIX data: {e}")
            return 18.5  # Fallback on error
